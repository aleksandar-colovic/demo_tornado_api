...
int main(int argc, char const *const *argv)
{
  ...
  amqp_socket_t *socket = amqp_tcp_socket_new(conn);
   if (!socket) die("Creating TCP socket failed");
  int status = amqp_socket_open(socket, "localhost", AMQP_PROTOCOL_PORT);
  if (status) die("Opening TCP socket  failed");

  die_on_amqp_error(amqp_login(conn, "/", 0, AMQP_DEFAULT_FRAME_SIZE, 0, AMQP_SASL_METHOD_PLAIN, "guest", "guest"), "Login failed");
  ...
  if (!amqp_basic_qos(conn, KChannel, 0, /*prefetch_count*/1, 0)) die_on_amqp_error(amqp_get_rpc_reply(conn), "basic.qos");
  if (!amqp_basic_consume(conn, KChannel, queueName, amqp_empty_bytes, false, /* auto ack*/ false, false, amqp_empty_table), "amqp_basic_consume failed");
  ...
  for (;;)
  {
    ...
    die_on_amqp_error(res, "amqp_consume_message failed");
    ...
    die_on_error(amqp_basic_publish(conn, replyChannel, amqp_empty_bytes, /* routing key*/ envelope.message.properties.reply_to, false, false, &props, amqp_cstring_bytes(comments.c_str())), "amqp_basic_publish failed");
    die_on_error(amqp_basic_ack(conn, replyChannel, envelope.delivery_tag, false), "amqp_basic_ack failed");
  ...
  die_on_amqp_error(amqp_channel_close(conn, KChannel, AMQP_REPLY_SUCCESS), "amqp_channel_close failed");
  die_on_amqp_error(amqp_connection_close(conn, AMQP_REPLY_SUCCESS), "amqp_connection_close failed");
  die_on_error(amqp_destroy_connection(conn), "amqp_destroy_connection failed");
